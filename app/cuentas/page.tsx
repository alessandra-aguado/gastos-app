  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8">
      <div className="flex justify-between items-end mb-6">
        ...
        <button onClick={() => setModalAbierto(true)} className="text-sm px-4 py-2 rounded-full border border-border hover:border-accent transition-colors">+ Agregar cuenta</button>
      </div>
      ...
      {modalAbierto && (
        <div className="fixed inset-0 bg-black/45 flex items-center justify-center p-5 z-50" onClick={() => setModalAbierto(false)}>
          <div className="w-[380px] bg-surface rounded-2xl p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <input className="w-full border border-border rounded-xl px-3 py-2 bg-background text-sm mb-2.5" placeholder="Cuenta corriente, Guardadito..." />
            <label className="text-xs text-muted block mb-1">Banco o emisor</label>
            <select className="w-full border border-border rounded-xl px-3 py-2 bg-background text-sm mb-3">
            ...
            {tipo === "cuenta" ? (
              <>
                ...
                <input type="number" placeholder="1200" className="w-full border border-border rounded-xl px-3 py-2 bg-background text-sm mb-2.5" />
                <label className="text-xs text-muted block mb-1">Nota (opcional)</label>
                <input className="w-full border border-border rounded-xl px-3 py-2 bg-background text-sm mb-2.5" placeholder="Cuenta compartida con mamá y hermana..." />
                ...
                <input type="number" defaultValue={3} className="w-full border border-border rounded-xl px-3 py-2 bg-background text-sm mb-3" />
                <button className="w-full py-2.5 bg-accent text-white rounded-full text-sm font-medium">Guardar cuenta</button>
              </>
            ) : (
